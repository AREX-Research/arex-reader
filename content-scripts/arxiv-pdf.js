(function(){function e(e){return e}function t(e){try{let t=new URL(e);return t.hostname!==`arxiv.org`&&!t.hostname.endsWith(`.arxiv.org`)?null:t.pathname.match(/^\/(?:abs|pdf|html)\/(.+?)(?:\.pdf)?$/i)?.[1]??null}catch{return null}}function n(e){try{let t=new URL(e).pathname;return/\.pdf$/i.test(t)||/^\/pdf\/.+/i.test(t)}catch{return!1}}function r(){let e=document.createElementNS(`http://www.w3.org/2000/svg`,`svg`);return e.classList.add(`mark`),e.setAttribute(`viewBox`,`0 0 388 337`),e.setAttribute(`fill`,`currentColor`),e.setAttribute(`aria-hidden`,`true`),e.innerHTML=`
  ...
  `,e}function i({collapseLabelOnNarrow:e=!1,density:t=`default`,hostId:n=`arex-reader-entry`,label:i=`速览`,openingLabel:a=`正在打开速览`,failureLabel:o=`暂时无法打开速览，请重试`,placement:s=`inline`,title:c,onActivate:l}){let u=document.createElement(`span`);u.id=n,u.dataset.collapseLabelOnNarrow=e?`true`:`false`,u.dataset.density=t,u.dataset.placement=s;let d=u.attachShadow({mode:`open`}),f=document.createElement(`style`);f.textContent=`
    :host {
      display: inline-flex;
      align-items: center;
      flex: 0 0 auto;
