(function(){function e(e){return e}function t(e){try{let t=new URL(e);return t.hostname!==`arxiv.org`&&!t.hostname.endsWith(`.arxiv.org`)?null:t.pathname.match(/^\/(?:abs|pdf|html)\/(.+?)(?:\.pdf)?$/i)?.[1]??null}catch{return null}}function n(e){try{let t=new URL(e).pathname;return/\.pdf$/i.test(t)||/^\/pdf\/.+/i.test(t)}catch{return!1}}var r={entry:2147483645,reader:2147483646,selection:2147483647};function i(){let e=document.createElementNS(`http://www.w3.org/2000/svg`,`svg`);return e.classList.add(`mark`),e.setAttribute(`viewBox`,`0 0 388 337`),e.setAttribute(`fill`,`currentColor`),e.setAttribute(`aria-hidden`,`true`),e.innerHTML=`
  ...
  `;let m=document.createElement(`button`);m.type=`button`,m.title=l,m.setAttribute(`aria-label`,l);let h=document.createElement(`span`);h.className=`label`,h.textContent=a;let g=document.createElement(`span`);return g.className=`status`,g.setAttribute(`role`,`status`),g.setAttribute(`aria-live`,`polite`),m.append(i(),h),m.addEventListener(`click`,async()=>{if(!m.disabled){m.disabled=!0,g.textContent=o;try{await u(),g.textContent=``,m.title=l}catch{g.textContent=s,m.title=s}finally{m.disabled=!1}}}),f.append(p,m,g),d}function o(){let e=`floating`,t=t=>(t.type===`READER_SURFACE_MODE_UPDATED`&&(e=t.mode),!1);return chrome.runtime.onMessage.addListener(t),chrome.runtime.sendMessage({type:`GET_READER_SURFACE_MODE`}).then(t=>{t.ok&&`readerSurfaceMode`in t&&(e=t.readerSurfaceMode)}).catch(()=>void 0),{open(t,n){return chrome.runtime.sendMessage({type:`OPEN_READER`,mode:n??e,...t})},dispose(){chrome.runtime.onMessage.removeListener(t)}}}function s(e){let t=e.trim().replace(/\.$/,``).toLowerCase();if(!t||/[\s/@]/.test(t))return null;try{return new URL(`https://${t}`).hostname.toLowerCase()}catch{return null}}function c(e){try{let t=new URL(e);return t.protocol!==`http:`&&t.protocol!==`https:`?null:s(t.hostname)}catch{return null}}function l(e,t){let n=c(t);return!!(n&&!e.disabledHosts.includes(n))}function u({currentUrl:e=()=>location.href,onChange:t}){let n=null,r=!1,i=null,a=()=>!!(n&&!r&&l(n,e())),o=()=>{let e=a();e!==i&&(i=e,t(e))},s=e=>(e.type===`WEB_SURFACE_SETTINGS_UPDATED`?(n=e.settings,o()):e.type===`WEB_SURFACES_DISMISSED_FOR_PAGE`&&(r=!0,o()),!1);return chrome.runtime.onMessage.addListener(s),o(),chrome.runtime.sendMessage({type:`GET_WEB_SURFACE_SETTINGS`}).then(e=>{e.ok&&`webSurfaceSettings`in e&&(n=e.webSurfaceSettings,o())}).catch(()=>void 0),{automaticSurfacesEnabled:a,dispose(){chrome.runtime.onMessage.removeListener(s)}}}function d(e){let t=e?.trim().toLowerCase()??``;return t===`zh`||t.startsWith(`zh-`)?`zh-CN`:`en`}function f(e,t){return e===`system`?d(t):e}function p(e){let t=!1,n={preference:`system`,locale:f(`system`,chrome.i18n.getUILanguage())},r=r=>{t||(r.locale!==n.locale||r.preference!==n.preference)&&(n=r,e(r))};e(n),chrome.runtime.sendMessage({type:`GET_UI_LANGUAGE`}).then(e=>{e.ok&&`uiLanguage`in e&&r(e.uiLanguage)}).catch(()=>void 0);let i=e=>(e.type===`UI_LANGUAGE_UPDATED`&&r(e.settings),!1);return chrome.runtime.onMessage.addListener(i),{dispose(){t=!0,chrome.runtime.onMessage.removeListener(i)}}}var m={common:{settings:`设置`,overview:`速览`,retry:`重试`,send:`发送`,collapse:`收起`,visitArex:`访问 AREX 主页`,openArex:`打开 AREX`,openArexChat:`打开 AREX 聊天`,collapseArexChat:`收起 AREX 聊天`,arexChatWindow:`AREX 聊天窗口`,openFailed:`暂时无法打开，请重试。`,openOverview:`打开 AREX 速览`,openPaperOverview:`打开 AREX 论文速览`,openVideoOverview:`打开 AREX 视频速览`,openingOverview:`正在打开速览`,openOverviewFailed:`暂时无法打开速览，请重试`},options:{documentTitle:`AREX Reader 设置`,account:`账户`,signedIn:`已登录 AREX`,signedOut:`未登录，可在阅读窗口中登录。`,checkingAccount:`正在读取登录状态…`,signOut:`退出登录`,languageHeading:`语言`,languageDescription:`用于界面和新生成的速览。`,system:`跟随系统`,english:`English`,chinese:`中文`,systemHint:`当前系统语言：中文`,fontHeading:`阅读字号`,fontDescription:`调整对话和速览的字号，小窗口和侧边栏会同步生效。`,fontSizes:{small:`小`,standard:`标准（默认）`,large:`大`},fontPreviewLabel:`预览`,fontPreview:`### 读懂论文的关键想法
